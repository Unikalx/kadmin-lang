    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
        ...
        <tr>
            ...
            <td class="tc" width="" data-field-name="type">
                Тип медиа
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='type'?'-':'';?>type" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-id_sect'){echo 'icon_chevron_up';}elseif($_GET['sort']=='type' ){echo 'icon_chevron_down';}else{echo 'type';};?>"></use></svg></a>
            </td>
        ...
        <?php $i=0; foreach($media as $m):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$m['id']?>">
                ...
                <td class="tc">
                    <a href="/kadmin/?t=<?=$table?>&c=change&id=<?=$m['id']?>" class="link_icon">
