...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-up-level-id="<?=$_GET['sect']?>" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name=""><?= constant(strtoupper($table))==1 ? 'ОТЗЫВЫ' : constant(strtoupper($table));?></span>
        <div class="per_page">
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
        ...
        <?php $i=0; foreach($testimonials as $testimonial):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$testimonial['id']?>" style="<?= $testimonial['checkbox'] == 1 ? "" : "background-color: rgba(247, 76, 76, 0.81);" ?>">
                <td class="tc" width="40">
                    <div class="row">
                ...
                <td class="tc"><?=$testimonial['id']?></td>
                <td class="tc"><?=$testimonial['name']?></td>
                <td class="tc">
                    <a href="/kadmin/?t=<?=$table?>&c=change&id=<?=$testimonial['id']?>" class="link_icon">
