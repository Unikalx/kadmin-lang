...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-up-level-id="<?=$_GET['sect']?>" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name=""><?= constant(strtoupper($table))==1 ? 'Баннер №1' : constant(strtoupper($table));?></span>
        <div class="per_page">
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
        ...
        <tbody>
        <?php $i=0; foreach($banner_1 as $banner):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$banner['id']?>">
                ...
                <td class="tc"><img src="image.php?width=80&height=80&cropratio=1:1&image=/pictures/<?=$table?>/<?=$banner['image1']?>" alt="<?=$banner['name']?>"></td>
                <td class="tc">
