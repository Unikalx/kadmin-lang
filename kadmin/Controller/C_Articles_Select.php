...
// конттроллер сторінки всіх Articles
class C_Articles_Select extends C_Base
{
	public $Articles;
	public $Categories;
	...
	//якщо є фільтрація по категоріях
		if(isset($_GET['cat'])){
			$cat = (int)$_GET['cat'];
			$this->where = " WHERE `id` IN (
									SELECT `id_article` FROM `article_in_cats` WHERE `id_category`='".$cat."' GROUP BY `id_article`
											)";
		}
	...
	//визначаємо параметри пагінації
		...
		if($this->pagination['count_pages'] > 1){
			...
			$this->Articles = $object->Array_where('articles',$this->where.' '.$this->order.' '.$this->limit);

			$this->Categories = $object->All_rows('article_cats');

	//форматуємо юрл по шаблону
		foreach ($this->Articles  as &$sectDb) {
			$sectDb['url'] = M_Columns::getFormatUrl($this->table, $vall = array(1 => "{category_url}",2 => $sectDb['url']));
		}
