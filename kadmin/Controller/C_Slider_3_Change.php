...
class C_Slider_3_Change extends C_Base
{
	...
	protected function OnInput()
	{
		...
		//видалення фотографії
			...
			$this->Slider_3_new = $_POST[$this->table];

			if($_GET['id']!='') {
				// используем метод класса Model для вывода секции по id
				$this->Slider_3 = $object->Row_by_id($this->table, $_GET['id']);
			}

	//зберігаємо данні в БД (checkbox)
			!isset($this->Slider_3_new['checkbox']) ? $this->Slider_3_new['checkbox']=0 : "";

			if($this->Slider_3['id'] == null) {

			//визначаємо останю позицію
				if($m_colums->ifIssetPosition($this->table))
					$this->Slider_3_new['position'] = $m_colums->getMaxPosition($this->table)+1;

			//___if Insert
				$this->Slider_3_new['id'] = $this->Slider_3['id'] = $msql->Insert($this->table, $this->Slider_3_new);
			}
		...
		//зберігаємо усі зображення, що прийшли з форми
			...
			// обновление фонової картинки
				...
				{
					$this->Slider_3_new[$key] = $m_colums->Upload_image($key,
														  $_FILES[$key],
														  $this->Slider_3['id']);
				}
				else
				{
					$this->Slider_3_new[$key] = $this->Slider_3[$key];
				}
			}
		//___if Update
			$msql->Update($this->table, $this->Slider_3_new, " `id`='" . $this->Slider_3['id'] . "'");


			if($_GET['id']!='' || isset($_POST['save_close'])){
				header("Location: /kadmin/?t=".$this->table."&c=select&page=1");
			}else{
				header("Location: /kadmin/?t=".$this->table."&c=change&id=".$this->Slider_3['id']);
			}
		}
	...
	//формуємо за допомогою шаблону юрла
		$this->Url['url'] = M_Columns::getFormatUrl($this->table, $vall = array(1  => $this->Slider_3['url']));
		if($_GET['id']==''){
			$this->Url['first-entrance'] = stripos($this->Url['url'], '{*}');
			$this->Url['url'] = str_replace('{*}','',$this->Url['url']);
		}else{
			$this->Url['first-entrance'] = stripos($this->Url['url'], $this->Slider_3['url']);
		}
