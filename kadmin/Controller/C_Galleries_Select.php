...
// конттроллер сторінки всіх Galleries
class C_Galleries_Select extends C_Base
{
	public  $Galleries;
	public  $GalleryCat;
	private $limit;
	...
	//якщо є фільтрація
		if($_GET['cat']!=''){
			$cat = (int)$_GET['cat'];
			$this->where = " WHERE `id_category`='$cat'";
		}
	...
	// виводимо секції
		...
		$this->Galleries = $object->Array_where($this->table, $this->where." ".$this->order." ".$this->limit);
		$getGalleryCats = $object->All_rows('gallery_cats');
		if ($getGalleryCats){
			$this->GalleryCat = $object->IndexBy($getGalleryCats,'id');
		}
		

	//форматуємо url по шаблону
		foreach ($this->Galleries  as &$gall) {
			$gall['url'] = M_Columns::getFormatUrl($this->table, $vall = array(1 => $this->GalleryCat[$gall['id_category']]['url'],2 => $gall['url']));
		}
	}
