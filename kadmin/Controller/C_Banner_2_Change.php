...
class C_Banner_2_Change extends C_Base
{
	...
	protected function OnInput()
	{
		...
		//видалення фотографії
			...
			$this->Banner_2_new = $_POST[$this->table];

			if($_GET['id']!='') {
				// используем метод класса Model для вывода секции по id
				$this->Banner_2 = $object->Row_by_id($this->table, $_GET['id']);
			}

	//зберігаємо данні в БД (checkbox)
			!isset($this->Banner_2_new['checkbox']) ? $this->Banner_2_new['checkbox']=0 : "";

			if($this->Banner_2['id'] == null) {

			//визначаємо останю позицію
				if($m_colums->ifIssetPosition($this->table))
					$this->Banner_2_new['position'] = $m_colums->getMaxPosition($this->table)+1;

			//___if Insert
				$this->Banner_2_new['id'] = $this->Banner_2['id'] = $msql->Insert($this->table, $this->Banner_2_new);
			}
		...
		//зберігаємо усі зображення, що прийшли з форми
			...
			// обновление фонової картинки
				...
				{
					$this->Banner_2_new[$key] = $m_colums->Upload_image($key,
														  $_FILES[$key],
														  $this->Banner_2['id']);
				}
				else
				{
					$this->Banner_2_new[$key] = $this->Banner_2[$key];
				}
			}
		//___if Update
			$msql->Update($this->table, $this->Banner_2_new, " `id`='" . $this->Banner_2['id'] . "'");


			if($_GET['id']!='' || isset($_POST['save_close'])){
				header("Location: /kadmin/?t=".$this->table."&c=select&page=1");
			}else{
				header("Location: /kadmin/?t=".$this->table."&c=change&id=".$this->Banner_2['id']);
			}
		}
	...
	//формуємо за допомогою шаблону юрла
		$this->Url['url'] = M_Columns::getFormatUrl($this->table, $vall = array(1  => $this->Banner_2['url']));
		if($_GET['id']==''){
			$this->Url['first-entrance'] = stripos($this->Url['url'], '{*}');
			$this->Url['url'] = str_replace('{*}','',$this->Url['url']);
		}else{
			$this->Url['first-entrance'] = stripos($this->Url['url'], $this->Banner_2['url']);
		}
