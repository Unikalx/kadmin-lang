...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-up-level-id="<?=$_GET['sect']?>" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name=""><?= constant(strtoupper($table))==1 ? 'ПОДПИСЧИКИ' : constant(strtoupper($table));?></span>
        <div class="per_page">
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
        ...
        <?php $i=0; foreach($subscribers as $subscriber):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$subscriber['id']?>">
                <td class="tc" width="40">
                    <div class="row">
                ...
                <td class="tc"><?=$subscriber['id']?></td>
                <td class="tc"><?=$subscriber['name']?></td>
                <td class="tc"><?=$subscriber['email']?></td>
                <td class="tc">
                    <a href="/kadmin/?t=<?=$table?>&c=change&id=<?=$subscriber['id']?>" class="link_icon">
