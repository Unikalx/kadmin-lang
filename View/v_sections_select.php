...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name="<?=$table?>"><?= constant(strtoupper($table))==1 ? 'СЕКЦИИ' : constant(strtoupper($table));?></span>
        <div class="per_page">
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>" data-table-sort="" data-up-level-id="">
        ...
        <tr>
            ...
            <!--
			<td class="tc" width="" data-field-name="landing">
                Лендинг
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='landing'?'-':'';?>landing" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-landing'){echo 'icon_chevron_up';}elseif($_GET['sort']=='landing' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
            <td class="tc" width="" data-field-name="has_subsect">
                Имеет подсекции?
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='has_subsect'?'-':'';?>has_subsect" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-has_subsect'){echo 'icon_chevron_up';}elseif($_GET['sort']=='has_subsect' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
			-->
        ...
        <tr id="tr<?=++$i;?>" data-field-id="<?=$section['id']?>">
            <td class="tc" width="40">
                <div class="row">
            ...
            <td class="tc"><?=$section['id']?></td>
            <td class="tc"><?=$section['name']?></td>
            <td class="tc"><a href="/<?=$section['url']?>">/<?=$section['url']?></a></td>
            <!--<td class="tc"><?=$section['landing']?></td>
            <td class="tc"><?=$section['has_subsect']?></td> -->
            <td class="tc">
                <a href="/kadmin/?t=<?=$table?>&c=change&id=<?=$section['id']?>" class="link_icon">
