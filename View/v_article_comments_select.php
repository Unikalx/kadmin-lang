    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
        ...
        <tr>
            ...
            <td class="tc" width="" data-field-name="text">
                Текст
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='text'?'-':'';?>text" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-text'){echo 'icon_chevron_up';}elseif($_GET['sort']=='text' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
        ...
        <?php $i=0; foreach($comments as $comment):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$comment['id']?>">
                ...
                <td class="tc">
                    <a href="/kadmin/?t=<?=$table?>&c=change&id=<?=$comment['id']?>" class="link_icon">
