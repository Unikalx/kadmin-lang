    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>" data-table-sort="" data-up-level-id="">
        ...
        <tr>
            ...
            <td class="tc" width="" data-field-name="login">
                Логин
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='login'?'-':'';?>login" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-login'){echo 'icon_chevron_up';}elseif($_GET['sort']=='login' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
            <td class="tc" width="" data-field-name="admin_mail">
                Email
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='admin_mail'?'-':'';?>admin_mail" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-admin_mail'){echo 'icon_chevron_up';}elseif($_GET['sort']=='admin_mail' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
            <td class="tc" width="" data-field-name="last_login">
                Последний вход
                <a href="/kadmin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='last_login'?'-':'';?>last_login" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-last_login'){echo 'icon_chevron_up';}elseif($_GET['sort']=='last_login' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
        ...
        <tr id="tr<?=++$i;?>" data-field-id="<?=$user['id_user']?>">
            ...
            <td class="tc">
                <a href="/kadmin/?t=<?=$table?>&c=change&id=<?=$user['id_user']?>" class="link_icon">
