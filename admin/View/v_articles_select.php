...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name=""><?= constant(strtoupper($table))==1 ? 'Список статей' : constant(strtoupper($table));?></span>
        <div class="per_page">
            ...
            <?php if (count ($categories) ) : ?>
                <select class="input ||  ml_filter">
                    ...
                    <?php foreach($categories as $cat):?>
                        <option value="<?=$cat['id']?>" <?=($_GET['cat']==$cat['id']) ? "selected" : ""?>><?=$cat['name']?></option>
                    <?php endforeach;?>
                </select>
            <?php endif;?>
        </div>
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>" data-table-sort="" data-up-level-id="">
        ...
        <tr id="tr<?=++$i;?>" data-field-id="<?=$article['id']?>">
            <td class="tc" width="40">
                <div class="row">
            ...
            <td class="tc"><?=$article['name']?></td>
            <td class="tc"><a href="#" onclick="return false;">/<?=$article['url']?></a></td>
            <td class="tc"><?=$article['date']?></td>
            <td class="tc">
                <a href="/admin/?t=<?=$table?>&c=change&id=<?=$article['id']?>" class="link_icon">
