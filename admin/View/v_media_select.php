...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-up-level-id="<?=$_GET['sect']?>" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name=""><?= constant(strtoupper($table))==1 ? 'Медиа' : constant(strtoupper($table));?></span>
        <div class="per_page">
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
           data-first-position="<?=$gallery_cats[0]['position']?>">
        <thead>
        <tr>
            ...
            <td class="tc" width="" data-field-name="type">
                Тип медиа
                <a href="/admin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='type'?'-':'';?>type" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-id_sect'){echo 'icon_chevron_up';}elseif($_GET['sort']=='type' ){echo 'icon_chevron_down';}else{echo 'type';};?>"></use></svg></a>
            </td>
        ...
        <tbody>
        <?php $i=0; foreach($media as $m):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$m['id']?>">
                <td class="tc" width="40">
                    <div class="row">
                ...
                <td class="tc"><?=$m['id']?></td>
                <td class="tc"><?=$m['name']?></td>
                <td class="tc"><?=$m['date']?></td>
                <td class="tc"><?=$type[$m['type']]?></td>
                <td class="tc">
                    <a href="/admin/?t=<?=$table?>&c=change&id=<?=$m['id']?>" class="link_icon">
