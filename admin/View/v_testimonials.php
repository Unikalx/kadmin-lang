...
<div class='wrapper' data-table-name="<?=$table?>" data-up-level-id="<?=$_GET['up_level_id']?>">
    <form data-edit-create action='/admin/?c=edit&t=<?=$table ?><?=$_GET['up_level_id'] ? '&up_level_id=' . $_GET['up_level_id'] : '' ?><?=$_GET['up_level_table'] ? '&up_level_table=' . $_GET['up_level_table'] : '' ?>' method='POST' class='clearfix validate_form || with_errors || inst_valid' enctype="multipart/form-data">
        ...
        <section>
            ...
            <span class='h1'><?=defined(strtoupper($table)) && constant(strtoupper($table)) != 1 ? constant(strtoupper($table)):$table?> <?=($action == 2)?'CREATE':'EDIT'?>
                <?php if (!in_array($table, array('faq', 'tags', 'subscribers', 'article_comments', 'article_cat')) ): ?>
                    <a href="/admin/?c=settings&t=<?=$table?>"><svg class='icon fr'><use xlink:href='View/img/svgdefs.svg#icon_settings'></use></svg></a>
                <?php endif; ?>
            </span>
