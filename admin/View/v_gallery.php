<div class='wrapper'>
    <form action='/admin/?c=edit&t=<?=$table ?><?=$_GET['up_level_id'] ? '&up_level_id=' . $_GET['up_level_id'] : '' ?><?=$_GET['up_level_table'] ? '&up_level_table=' . $_GET['up_level_table'] : '' ?>'  method='POST' class='clearfix' enctype="multipart/form-data">
            ...
            <?php foreach ($columns as $pr): ?>
                <?php if (!(in_array($pr['field_name'], ['url', 'id', 'position'])) && strpos($pr['field_name'], 'title') === false && strpos($pr['field_name'], 'keywords') === false && strpos($pr['field_name'], 'description') === false): ?>
                    ...
                    <?php if ($pr['field_type'] == 'file'): ?>
                        <div data-table="<?=$table?>" data-field="<?=$pr['field_name']?>" data-<?=$table . '_' . $pr['field_name']?> class='pre_input' data-image-url="/admin/picture/<?=$table?>/<?=$pr['value'] ?>"><?=$pr['field_title']?>
...
<script>
    $(".dropzone").dropzone({
        url: '/admin/?c=edit&t=<?=$table ?>&r=<?=$columns['id']['value'] ?>',
        maxFilesize: 3,
