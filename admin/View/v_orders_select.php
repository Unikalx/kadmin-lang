    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>" data-table-sort="" data-up-level-id="">
        ...
        <tr>
            ...
            <td class="tc" width="" data-field-name="sum">
                Всего
                <a href="/admin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='sum'?'-':'';?>sum" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-sum'){echo 'icon_chevron_up';}elseif($_GET['sort']=='sum' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
        ...
        <tr id="tr<?=++$i;?>" data-field-id="<?=$order['id_order']?>">
            ...
            <td class="tc">
                <a href="/admin/?t=<?=$table?>&c=change&id=<?=$order['id_order']?>" class="link_icon">
