    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>" data-table-sort="" data-up-level-id="">
        ...
        <tr>
			...
			<td class="tc" width="" data-field-name="landing">
                Лендинг
                <a href="/admin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='landing'?'-':'';?>landing" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-landing'){echo 'icon_chevron_up';}elseif($_GET['sort']=='landing' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
            <td class="tc" width="" data-field-name="has_subsect">
                Имеет подсекции?
                <a href="/admin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='has_subsect'?'-':'';?>has_subsect" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-has_subsect'){echo 'icon_chevron_up';}elseif($_GET['sort']=='has_subsect' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
			-->
        ...
        <tr id="tr<?=++$i;?>" data-field-id="<?=$section['id']?>">
            ...
            <td class="tc">
                <a href="/admin/?t=<?=$table?>&c=change&id=<?=$section['id']?>" class="link_icon">
