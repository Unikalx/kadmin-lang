...
<section data-sfield-name="" data-sfield-value="" data-table-has-url="1" data-up-level-id="<?=$_GET['sect']?>" data-table-name="<?=$table?>">
    <div class="underlined h2">
        <span data-showed-table-name=""><?= constant(strtoupper($table))==1 ? 'Комментарии' : constant(strtoupper($table));?></span>
        <div class="per_page">
...
<!--                <a class="button || outline" href="/admin/?t=--><?//=$table?><!--&c=change">Добавить новое</a>-->
    ...
    <table class="table || checkbox_table" data-table-name="<?=$table?>"
        ...
        <tr>
            ...
            <td class="tc" width="" data-field-name="text">
                Текст
                <a href="/admin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='text'?'-':'';?>text" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-text'){echo 'icon_chevron_up';}elseif($_GET['sort']=='text' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
            <td class="tc" width="" data-field-name="date">
                Дата
                <a href="/admin/?t=<?=$table?>&c=select&sort=<?=$_GET['sort']=='date'?'-':'';?>date" title="Sort items" class="sort"><svg class="icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="View/img/svgdefs.svg#<?php if($_GET['sort']=='-date'){echo 'icon_chevron_up';}elseif($_GET['sort']=='date' ){echo 'icon_chevron_down';}else{echo 'icon_sort';};?>"></use></svg></a>
            </td>
        ...
        <?php $i=0; foreach($comments as $comment):?>
            <tr id="tr<?=++$i;?>" data-field-id="<?=$comment['id']?>">
                <td class="tc" width="40">
                    <div class="row">
                ...
                <td class="tc"><?=$comment['name']?></td>
                <td class="tc"><?=strlen($comment['value'])>20 ? (substr($comment['value'], 0, strpos($comment['value'], ' ', 20)) . '...') :$comment['value'];?></td>
                <td class="tc"><?=$comment['date']?></td>
                <td class="tc">
                    <a href="/admin/?t=<?=$table?>&c=change&id=<?=$comment['id']?>" class="link_icon">
