...
// конттроллер сторінки всіх Comments
class C_Article_Comments_Select extends C_Base
{
	public $Comments;
	...
	//якщо є сортування
		...
		if(isset($_GET['article']) && $_GET['article']!=''){
			...
			$this->where = "WHERE `article_id`='$article_id' ";
		}
	...
	// виводимо секції
		...
		$this->Comments = $object->Array_where($this->table, $this->where." ".$this->order." ".$this->limit);
	}
