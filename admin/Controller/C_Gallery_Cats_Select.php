...
// конттроллер сторінки всіх GalleryCats
class C_Gallery_Cats_Select extends C_Base
{
	public $GalleryCats;
	...
	// pagination
		...
		$this->GalleryCats = $object->Array_where($this->table, $this->where." ".$this->order." ".$this->limit);
		//форматуємо url по шаблону
		foreach ($this->GalleryCats  as &$g_c) {
			$g_c['url'] = M_Columns::getFormatUrl($this->table, $vall = array(1 => $g_c['url']));
		}
	}
